...
import { LayoutDashboard, UserCircle, Settings, LogOut, Search, ChevronDown, Menu, Users, Bell, Trophy, X } from "lucide-react";
...
interface SidebarProps {
  /** Controlled from outside on mobile — passed by MobileHeader */
  mobileOpen?: boolean;
  onMobileClose?: () => void;
}
...
export default function Sidebar({ mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <aside
      ...
      className={`
        relative flex-shrink-0 h-full bg-(--card) border-r border-(--brd) flex flex-col overflow-hidden
    ...
    >
      <style>{`
        ...
        .sd-anim {
          animation: slideDown 340ms cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        @keyframes slideDown {
      ...
      <div className={`flex items-center border-b border-(--brd) flex-shrink-0 ${collapsed ? "justify-center px-4 py-[18px]" : "justify-between px-5 py-[18px]"}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="lg:hidden w-9 h-9 rounded-xl bg-(--bg) border border-(--brd) flex items-center justify-center text-(--t2) hover:text-red-500 hover:border-red-500/40 transition-all"
          >
            <X size={16} />
          </button>
          ...
          <button
            ...
            className={`
              hidden lg:flex w-9 h-9 rounded-xl bg-(--bg) border border-(--brd)
              items-center justify-center collapse-btn
    ...
    <>
      ...
      {mobileOpen && (
        <>
          ...
          {/* Drawer */}
          <div
            className="fixed inset-y-0 left-0 z-50 lg:hidden flex h-full"
            style={{ animation: "slideInLeft 380ms cubic-bezier(0.16, 1, 0.3, 1) forwards" }}
          >
            <style>{`
...
function NavItem({ icon, label, active, collapsed, onClick, suffix }: any) {
  return (
    <button
      onClick={onClick}
      className={`
        flex items-center gap-3 rounded-xl text-sm font-bold w-full
        ${collapsed ? "justify-center p-3" : "px-4 py-3"}
        ${active ? "bg-blue-600 text-white shadow-lg" : "text-(--t2) hover:bg-(--bg)"}
      `}
      style={{ transition: "background 220ms ease, color 220ms ease, box-shadow 220ms ease, transform 100ms ease" }}
