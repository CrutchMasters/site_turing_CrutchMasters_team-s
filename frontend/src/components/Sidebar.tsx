  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Collapse toggle — same look as settings button on main page */}
        <button
        ...
        <NavItem
          ...
          suffix={
            !collapsed ? (
              <ChevronDown
                size={13}
                className={`transition-transform flex-shrink-0 ${isSettingsPanelOpen ? "rotate-180" : ""}`}
              />
            ) : undefined
          }
        />
        ...
        {isSettingsPanelOpen && !collapsed && (
          <div className="sd-anim mx-1 bg-(--bg) border border-(--brd) rounded-2xl p-4 space-y-4">
                ...
                {(["en", "ru", "ua"] as const).map(lang => (
                  <button
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-black rounded-lg transition-all ${
                      locale === lang ? "bg-(--bg) shadow-sm text-blue-600" : "text-(--t2) hover:text-blue-400"
                    }`}
                  >
                    {lang.toUpperCase()}
                  </button>
            ...
            <div className="pt-2 border-t border-(--brd)">
              <span className="text-[10px] font-black text-(--t2) uppercase tracking-tighter block mb-0.5">
                {t.settings.status}:
              </span>
...
function NavItem({
  ...
  return (
    <button
      ...
      className={`
        flex items-center gap-3 rounded-xl text-sm font-bold spr transition-all w-full
