...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/notifications?limit=20`, {
  ...
  const handleNotificationsClick = () => {
    if (collapsed) {
      // Expand sidebar first, then open panel
      expand();
  ...
  const handleSettingsClick = () => {
    if (collapsed) {
      // Expand sidebar first, then open settings
      expand();
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-(--brd) flex-shrink-0 px-3 py-1">
        ...
        {isNotificationsPanelOpen && !collapsed && (
          <div className="sd-anim border-t border-(--brd)">
            ...
            <button
              onClick={() => go("/profile")}
              className="w-full py-2.5 text-[10px] font-black uppercase tracking-widest text-blue-500 hover:bg-blue-500/10 transition-colors border-t border-(--brd) rounded-b-2xl"
            >
              {t.sidebar.notifMore}
            </button>
          </div>
      ...
      <nav className="flex-1 flex flex-col gap-1 p-3 overflow-y-auto overflow-x-hidden">
        ...
        <NavItem icon={<Trophy size={18} />}          label={t.sidebar.tournaments} active={pathname === "/tournaments" || pathname?.startsWith("/tournaments/")} collapsed={collapsed} onClick={() => go("/tournaments")} />
        <NavItem icon={<Users size={18} />}           label={t.sidebar.teams}       active={pathname === "/teams" || pathname?.startsWith("/teams/")}  collapsed={collapsed} onClick={() => go("/teams")} />
