...
import { Mail, Send, Check, X, Loader, Clock, RefreshCw, Globe } from "lucide-react";
...
export default function TournamentJuryEmailInvitePanel({ tournamentId, tournamentName }: Props) {
  ...
  const fetchInvitations = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/tournaments/${tournamentId}/email-invitations`,
        { headers: authHeader() },
      );
      if (res.ok) {
  ...
  const sendInvite = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/tournaments/${tournamentId}/invite-by-email`,
        {
          method: "POST",
          headers: authHeader(),
  ...
  const tournamentLabel = tournamentName ?? "турнір";

  return (
    <div className="bg-(--card) rounded-2xl sm:rounded-[2.5rem] border border-amber-500/20 shadow-sm overflow-hidden">

      {/* Header */}
      <div className="w-full flex items-center gap-3 px-6 sm:px-8 py-4 border-b border-amber-500/10">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-black uppercase tracking-widest text-amber-500">
            Email-запрошення журі · Весь турнір
          </p>
        ...
        <button
          ...
          disabled={loading}
          className="w-7 h-7 rounded-lg bg-(--bg) border border-(--brd) flex items-center justify-center text-(--t2) hover:text-amber-500 transition-all"
          title="Оновити"
        >
      ...
      <div className="p-6 sm:p-8 space-y-5">
        ...
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-(--t2) mb-2 flex items-center gap-1.5">
            <Send size={10} className="text-amber-500" /> Надіслати запрошення на турнір
          </p>
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-(--brd) bg-(--bg) text-(--t1) focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 outline-none text-sm transition-all"
              />
            </div>
            <button
              type="button"
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-amber-500 text-white font-black text-[11px] uppercase tracking-widest hover:bg-amber-600 transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
            >
          ...
          <p className="mt-1.5 text-[10px] text-(--t2) font-medium">
            Отримувач отримає лист з посиланням на реєстрацію. Роль «журі» та доступ до всіх раундів{" "}
            <span className="font-black">«{tournamentLabel}»</span> видаються автоматично.
          </p>
        </div>
        ...
        {invitations.length === 0 && !loading && (
          <div className="text-center py-5">
            <Globe className="w-9 h-9 text-(--t2) opacity-25 mx-auto mb-2" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-(--card) rounded-2xl border border-amber-500/30 shadow-2xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] font-bold text-(--t2) mt-0.5">
                  Лист з посиланням для реєстрації на весь турнір
                </p>
              </div>
            </div>
            <div className="px-4 py-3 rounded-xl bg-amber-500/5 border border-amber-500/15">
              ...
              <p className="text-[10px] font-bold text-(--t2) mt-0.5">
                Отримає доступ до всіх раундів:{" "}
                <span className="text-(--t1)">{tournamentLabel}</span>
              </p>
            </div>
