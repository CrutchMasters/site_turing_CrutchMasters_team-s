...
export default function UserProfile() {
  ...
  const go = (path: string) => { setIsSidebarOpen(false); router.push(path); };
  ...
  return (
    ...
    <p className="flex items-center gap-3 font-medium"><User size={16} className="text-blue-600 flex-shrink-0"/><span className="text-(--t2)">Ім'я:</span> <span className="font-bold">Антон Петров</span></p>
    <p className="flex items-center gap-3 font-medium"><Mail size={16} className="text-blue-600 flex-shrink-0"/><span className="text-(--t2)">Email:</span> <span className="font-bold break-all">a.petrov@tournament.com</span></p>
    <p className="flex items-center gap-3 font-bold text-blue-600"><Shield size={16} className="flex-shrink-0"/><span>Роль:</span> <span className="uppercase tracking-wider">Team Lead</span></p>
    </div>
    </div>
    ...
    <div className="pt-3 border-t border-(--brd) text-[9px] font-bold uppercase tracking-widest text-(--t2) text-center sm:text-left">
    ...
    {[
      ...
      <p className="text-[10px] text-(--t2) font-bold uppercase tracking-tighter">{m.role}</p>
      </div>
      </div>
    ...
    <button className="w-full mt-5 py-4 border-2 border-dashed border-(--brd) rounded-2xl text-(--t2) font-bold text-[10px] uppercase tracking-widest hover:bg-blue-600/5 hover:border-blue-600/30 hover:text-blue-600 transition-all">
    + Додати учасника
    </button>
    </section>
    ...
    {[
      ...
      <div>
      <p className="font-bold text-sm leading-none mb-1">{s.task}</p>
      <p className="text-[9px] font-bold text-(--t2) uppercase tracking-wider">{s.time}</p>
      </div>
      </div>
      <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-md bg-(--card) border border-(--brd) ${s.color}`}>
      {i===0?"success":i===1?"pending":"failed"}
      </span>
      </div>
    ))}
    </div>
    <div className="mt-6 p-5 bg-blue-600/5 rounded-3xl border border-blue-600/10 flex items-center justify-between">
    <div>
