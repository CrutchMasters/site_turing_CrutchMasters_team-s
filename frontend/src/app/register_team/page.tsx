              ...
              <form className="space-y-10 sm:space-y-12" onSubmit={e => e.preventDefault()}>
                ...
                <section className="space-y-4 sm:space-y-6">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    ...
                    <span className="text-[10px] font-bold text-(--t2) uppercase tracking-widest bg-(--bg) px-3 py-1 rounded-full border border-(--brd)">{participants.length} / 10</span>
                  </div>
                    ...
                    {participants.map((p, idx) => (
                      <div key={p.id} className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-(--bg)/50 border border-(--brd) relative group hover:border-blue-600/30 transition-all">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input type="text" placeholder="ПІБ" value={p.name} onChange={e => updateParticipant(p.id, "name", e.target.value)} className={inputClass} />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] font-black text-(--t2) uppercase tracking-[0.15em] ml-1">Email</label>
                          <div className="relative">
                            <input type="email" placeholder="member@example.com" value={p.email} onChange={e => updateParticipant(p.id, "email", e.target.value)}
                        ...
                        {participants.length > 2 && (
                          <button type="button" onClick={() => removeParticipant(p.id)}
                            className="absolute top-3 right-3 w-6 h-6 rounded-full bg-(--bg) border border-(--brd) text-(--t2) hover:text-red-500 hover:border-red-500/30 transition-all text-xs font-black opacity-0 group-hover:opacity-100 flex items-center justify-center">×</button>
                        )}
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4">
                  <button type="submit" className="bg-blue-600 text-white px-8 sm:px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 transition-all active:scale-95 shadow-lg shadow-blue-600/20">
                    Створити команду
                  </button>
                  <button type="button" onClick={() => router.back()} className="bg-(--bg) border border-(--brd) text-(--t2) px-8 sm:px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-(--card) transition-all">
                    Скасувати
                  </button>
                </div>
              </form>
