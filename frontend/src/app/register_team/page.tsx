...
import {
  ...
  Send, Mail, Check, Loader, AlertCircle, Camera, ShieldOff,
} from "lucide-react";
...
function UserSearchDropdown({
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    try {
      ...
      .eq("status", "active")
      // Exclude restricted roles from search results so they can't be invited
      .not("role", "in", `(${RESTRICTED_ROLES.join(",")})`)
      .limit(12);
...
export default function RegisterTeamPage() {
  ...
  useEffect(() => {
    ...
    if (!isLoading && isRestricted) router.push("/teams");
    ...
    useEffect(() => {
      ...
      const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
          ...
          if (members.length > 0) {
            ...
            if (failed > 0) console.warn(`${failed} invitations failed to send`);
          }
      ...
      // ── Restricted role — show access denied before redirect fires ──
      if (isRestricted) {
        return (
          <div className="min-h-screen bg-(--bg) flex items-center justify-center">
          <div className="w-8 h-8 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" />
