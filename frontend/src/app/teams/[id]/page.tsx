...
import { useT } from "@/context/LanguageContext"; // Импорт хука
import { supabase } from "@/lib/supabase";
import Sidebar from "@/components/Sidebar";
import MobileHeader from "@/components/MobileHeader";
import {
    Users, Crown, ChevronRight, ArrowLeft,
    Star, Send, MessageSquare, Calendar,
...
function CopyButton({ text, t }: { text: string; t: any }) {
    ...
    return (
        <button onClick={handle} title={t.teams.copyId}
...
export default function TeamProfilePage() {
    ...
    const { t } = useT(); // Инициализация переводов
    ...
    useEffect(() => {
        ...
        const fetchTeam = async () => {
            ...
            try {
                ...
                if (teamErr || !teamData) throw new Error(t.teams.notFound);
                setTeam(teamData);
        ...
        <div className="flex h-screen overflow-hidden bg-(--bg) text-(--t1) transition-colors duration-300">
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                <MobileHeader
                    ...
                    title={team?.name ?? t.teams.profileTitle}
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 lg:p-12 relative z-10">
                    ...
                    <nav className="flex items-center gap-2 text-[10px] font-black mb-6 uppercase tracking-widest text-(--t2)">
                        ...
                        <button onClick={() => router.push("/teams")} className="hover:text-blue-600 transition-colors">{t.teams.breadcrumb}</button>
                        <ChevronRight size={10} />
                        <span className="text-(--t1) truncate max-w-[120px]">{isLoading ? "..." : team?.name ?? t.teams.profileTitle}</span>
                    </nav>
                        ...
                        <div className="max-w-3xl space-y-5">
                                            ...
                                            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-1">
                                                ...
                                                {isMyTeam && (
                                                    <span className="text-[9px] font-black uppercase bg-amber-500/10 text-amber-500 border border-amber-500/20 px-2.5 py-1 rounded-lg flex items-center gap-1">
                                                        <Star size={9} className="fill-amber-500" /> {t.teams.myTeamBadge}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 mt-3">
                                                <div className="flex items-center gap-1.5 text-(--t2)">
                                                    ...
                                                    <span className="text-[11px] font-black uppercase tracking-wider">
                                                        {allMembers.length} {t.teams.members}
                                                    </span>
                                                </div>
                                                {team.created_at && (
                                                    <div className="flex items-center gap-1.5 text-(--t2)">
                            ...
                            {(team.telegram_url || team.discord_url) && (
                                <section className="cdIn bg-(--card) rounded-2xl sm:rounded-[2.5rem] border border-(--brd) shadow-sm p-6 sm:p-8" style={{ animationDelay: "60ms" }}>
                                    <h2 className="text-xs font-black uppercase tracking-widest text-(--t2) mb-4">{t.teams.socialTitle}</h2>
                                    <div className="flex flex-wrap gap-3">
                                        ...
                                        {team.discord_url && (
                                            ...
                                            >
                                                <MessageSquare size={14} /> {t.teams.discord}
                                                <ExternalLink size={11} className="opacity-60" />
                                            </a>
                                        )}
                                    </div>
                            ...
                            {captain && (
                                <section className="cdIn bg-(--card) rounded-2xl sm:rounded-[2.5rem] border border-amber-500/20 shadow-sm overflow-hidden" style={{ animationDelay: "90ms" }}>
                                    <div className="flex items-center gap-3 px-6 sm:px-8 py-4 border-b border-(--brd) bg-amber-500/5">
                                        ...
                                        <h2 className="text-xs font-black uppercase tracking-widest text-amber-500">{t.teams.captainTitle}</h2>
                                    </div>
                                    <div className="p-6 sm:p-8">
                                        <MemberRow member={captain} isCaptain t={t} onClick={() => router.push(captain.id === user?.id ? "/profile" : `/user/${captain.id}`)} />
                                    </div>
                                </section>
                            )}
                            ...
                            <section className="cdIn bg-(--card) rounded-2xl sm:rounded-[2.5rem] border border-(--brd) shadow-sm overflow-hidden" style={{ animationDelay: "130ms" }}>
                                <div className="flex items-center justify-between px-6 sm:px-8 py-4 border-b border-(--brd)">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h2 className="text-xs font-black uppercase tracking-widest text-(--t1)">{t.teams.membersTitle}</h2>
                                    </div>
                                ...
                                <div className="divide-y divide-(--brd)">
                                    {members.length === 0 ? (
                                        <div className="px-6 sm:px-8 py-10 text-center">
                                            <p className="text-[11px] font-black uppercase tracking-widest text-(--t2)">{t.teams.noMembers}</p>
                                        </div>
                                    ) : (
                                        members.map((m, i) => (
...
function MemberRow({ member, isCaptain, onClick, t }: { member: TeamMember; isCaptain?: boolean; onClick?: () => void; t: any }) {
        ...
        >
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {member.status === "active" && (
                    <span className="w-2 h-2 rounded-full bg-green-500 flex-shrink-0" title={t.profile.active} />
                )}
