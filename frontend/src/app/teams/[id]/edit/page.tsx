...
import { useT } from "@/context/LanguageContext"; // Импортируем хук
...
export default function EditTeamPage() {
    ...
    const { t } = useT(); // Инициализируем переводы
    ...
    useEffect(() => {
        ...
        const fetchTeam = async () => {
            ...
            try {
                ...
                setError(e.message ?? t.teams.errorLoad);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleAddMember = async (member: TeamMember) => {
        ...
        try {
            ...
            setError(e.message ?? t.teams.errorAdd);
        } finally {
            setAddingId(null);
        }
    };
    ...
    const handleRemoveMember = async (member: TeamMember) => {
        ...
        try {
            ...
            setError(e.message ?? t.teams.errorRemove);
        } finally {
            setRemovingId(null);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setError(e.message ?? t.teams.errorSave);
        } finally {
            setIsSaving(false);
        }
    };
            ...
            {confirmRemove && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-(--bg)/70 backdrop-blur-md p-4">
                    <div className="modal bg-(--card) border border-red-500/30 rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
                        ...
                        <h2 className="text-base font-black text-(--t1) uppercase mb-1">{t.teams.removeConfirmTitle}</h2>
                        <p className="text-sm text-(--t2) mb-5">
                            {t.teams.removeConfirmText} <span className="font-black text-(--t1)">{confirmRemove.username}</span> {t.teams.removeFromTeam}?
                        </p>
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 lg:p-12 relative z-10">
                    ...
                    <nav className="flex items-center gap-2 text-[10px] font-black mb-6 uppercase tracking-widest text-(--t2)">
                        <button onClick={() => router.push("/dashboard")} className="hover:text-blue-600 transition-colors">{t.nav.home}</button>
                        <ChevronRight size={10} />
                        <button onClick={() => router.push("/my_teams")} className="hover:text-blue-600 transition-colors">{t.teams.myTeamsBtn}</button>
                        <ChevronRight size={10} />
                        ...
                        <ChevronRight size={10} />
                        <span className="text-(--t1)">{t.teams.editLabel}</span>
                    </nav>
                    ...
                    <div className="max-w-3xl space-y-6">
                        ...
                        <section className="cdIn bg-(--card) rounded-2xl sm:rounded-[2.5rem] border border-(--brd) shadow-sm overflow-hidden">
                            <div className="flex items-center gap-3 px-6 sm:px-8 py-4 border-b border-(--brd)">
                                ...
                                <h2 className="text-xs font-black uppercase tracking-widest text-(--t1)">{t.teams.basicInfo}</h2>
                            </div>
                            <div className="p-6 sm:p-8 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder={t.teams.orgPlaceholder}
                                        className={inputClass}
                                        maxLength={128}
                                    />
                                </div>
                                ...
                                <div className="flex justify-end pt-2">
                                    ...
                                    >
                                        {isSaving ? (
                                            <><Loader size={14} className="animate-spin" /> {t.common.saving}</>
                                        ) : saveSuccess ? (
                            ...
                            {members.length < 10 && (
                                <div className="px-6 sm:px-8 py-5 border-t border-(--brd) bg-(--bg)/40">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-(--t2) mb-3 flex items-center gap-2">
                                        <UserPlus size={12} className="text-blue-600" /> {t.teams.addMemberLabel}
                                    </p>
                        ...
                        <div className="fuIn pb-4" style={{ animationDelay: "200ms" }}>
                            ...
                            >
                                <ArrowLeft size={14} /> {t.teams.backToTeam}
                            </button>
                        </div>
