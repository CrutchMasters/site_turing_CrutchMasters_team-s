...
export default function TournamentEditPage() {
    ...
    useEffect(() => {
        ...
        const handleSave = async (e: React.FormEvent) => {
            ...
            if (!name.trim()) { setError(t.tourney?.errNameRequired ?? "Назва обов'язкова"); return; }
            if (!startDate)   { setError(t.tourney?.errStartRequired ?? "Дата старту обов'язкова"); return; }
            ...
            try {
                ...
                setSuccess(t.tourney?.successSaved ?? "Зміни збережено ✓");
                await fetchTourney();
            } catch (e: any) {
        ...
        const handleDelete = async () => {
            ...
            try {
                ...
                setError(e?.message ?? (t.tourney?.deleteError ?? "Не вдалося видалити турнір"));
        ...
        if (!tourney) {
            return (
                ...
                <p className="font-black text-(--t1) uppercase">{t.tourney?.errNotFound ?? "Турнір не знайдено"}</p>
                <button onClick={() => router.push("/tournaments")} className="text-blue-600 text-sm font-bold">{t.tourney?.backToTournaments ?? "← До турнірів"}</button>
                </div>
            );
        }
        ...
        return (
            ...
            <style jsx global>{`
                ...
                title={t.tourney?.mobileTitle ?? "Редагування турніру"}
                ...
                <button onClick={() => router.push("/tournaments")} className="hover:text-blue-600 transition-colors">{t.tourney?.breadcrumbTournaments ?? "Турніри"}</button>
                <ChevronRight size={10} />
                ...
                <ChevronRight size={10} />
                <span className="text-(--t1)">{t.tourney?.breadcrumbEdit ?? "Редагування"}</span>
                </nav>
                ...
                {t.tourney?.pageTitle ?? "Редагування турніру"}
                </h1>
                ...
                <label className={label10}>{t.tourney?.nameLabel ?? "Назва турніру"}</label>
                <span className="text-[9px] font-black uppercase text-red-500 flex items-center gap-1">
                ...
                <div>
                <label className={`block ${label10} mb-2`}>{t.tourney?.rulesLabel ?? "Опис / Правила"}</label>
                <RichTextEditor
                value={rules}
                onChange={setRules}
                ...
                <span className="text-xs font-black uppercase tracking-widest text-(--t2)">{t.tourney?.block2regTeams ?? "Реєстрація команд"}</span>
                </div>
                <div className="p-6 space-y-4 flex-1">
                <DateTimePair label={t.tourney?.regStart ?? "Початок реєстрації"} dateVal={regFromDate} onDate={setRegFromDate} timeVal={regFromTime} onTime={setRegFromTime} />
                <div className="border-t border-(--brd)" />
                <DateTimePair label={t.tourney?.regEnd ?? "Кінець реєстрації"} dateVal={regToDate} onDate={setRegToDate} timeVal={regToTime} onTime={setRegToTime} />
                </div>
                </div>
                ...
                <span className="text-xs font-black uppercase tracking-widest text-(--t2)">{t.tourney?.block2startDates ?? "Дати старту"}</span>
                </div>
                <div className="p-6 space-y-4 flex-1">
                <DateTimePair label={t.tourney?.tourStart ?? "Початок турніру"} dateVal={startDate} onDate={setStartDate} timeVal={startTime} onTime={setStartTime} required />
                <div className="border-t border-(--brd)" />
                ...
                <div className="flex items-center justify-between">
                <p className="text-[9px] font-black uppercase tracking-widest text-(--t2)">{t.tourney?.roundCountLabel ?? "Кількість раундів"}</p>
                <p className="text-[9px] font-black uppercase tracking-widest text-(--t2)">
                {t.tourney?.roundSelected ?? "Вибрано:"} <span className="text-blue-500">{roundCount}</span>{" "}
                {roundCount === 1
                    ? (t.tourney?.roundWord_1 ?? "раунд")
                    : roundCount < 5
                    ? (t.tourney?.roundWord_2 ?? "раунди")
                    : (t.tourney?.roundWord_5 ?? "раундів")}
                    </p>
                    </div>
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-(--t2) flex-1">{t.tourney?.teamsBlock ?? "Команди"}</span>
                    <span className="text-[9px] font-bold text-(--t2) bg-(--bg) border border-(--brd) px-2 py-0.5 rounded-full">{t.tourney?.optional ?? "Опціонально"}</span>
                    </div>
                    <div className="p-5 flex flex-col gap-3 flex-1">
                    <p className="text-[9px] font-black uppercase tracking-widest text-(--t2)">{t.tourney?.teamCountLabel ?? "Кількість команд"}</p>
                    ...
                    {saving
                        ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> {t.tourney?.saving ?? "Збереження..."}</>
                        : <><Save size={15} /> {t.tourney?.saveBtn ?? "Зберегти зміни"}</>
                    }
                    </button>
                    ...
                    <Trash2 size={15} /> {t.tourney?.deleteBtn ?? "Видалити"}
                    </button>
                    </div>
                    ...
                    {showDeleteModal && (
                        ...
                        {t.tourney?.deleteConfirmTitle ?? "Видалити турнір?"}
                        </h2>
                        </div>
                        <p className="text-sm text-(--t2) leading-relaxed">
                        {t.tourney?.deleteConfirmText ?? "Цю дію неможливо скасувати. Всі раунди та реєстрації команд будуть видалені назавжди."}
                        </p>
                        <div className="px-4 py-3 bg-(--bg) border border-(--brd) rounded-2xl">
                        <p className="text-[10px] font-black uppercase tracking-widest text-(--t2) mb-1">{t.tourney?.deleteConfirmName ?? "Турнір:"}</p>
                        ...
                        {t.tourney?.deleteCancelBtn ?? "Скасувати"}
                        </button>
                        <button
                        type="button"
                        ...
                        {deleting
                            ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> {t.tourney?.deleteConfirming ?? "Видалення..."}</>
                            : <><Trash2 size={14} /> {t.tourney?.deleteConfirmBtn ?? "Так, видалити"}</>
                        }
                        </button>
                        </div>
                        </div>
