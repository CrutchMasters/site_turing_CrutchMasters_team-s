...
import {
    ...
    CheckCircle, Clock, Layers, Zap, Users, ArrowLeft, Trash2, ImageIcon, Upload, X, Lock, Star,
} from "lucide-react";
...
// ── Mobile Tab System ─────────────────────────────────────────────────────────
type MobileTab = 'general' | 'rounds' | 'jury';
...
function MobileTabBar({
    ...
    return (
        <div className="xl:hidden sticky top-0 z-20 bg-(--bg)/90 backdrop-blur-md border-b border-(--brd) px-4 py-2">
            <div className="flex gap-1 bg-(--card) p-1.5 rounded-2xl border border-(--brd)">
                {([
                    { key: 'general', label: 'Загальне', icon: <Trophy size={12} /> },
                    { key: 'rounds',  label: 'Раунди',  icon: <Layers size={12} />, badge: roundsHaveErrors },
                    { key: 'jury',    label: 'Журі',    icon: <Star size={12} /> },
                ] as { key: MobileTab; label: string; icon: React.ReactNode; badge?: boolean }[]).map(tab => (
                    <button
                        key={tab.key}
                        type="button"
                        onClick={() => onChange(tab.key)}
                        className={`relative flex-1 flex items-center justify-center gap-1.5 py-2.5 px-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                            active === tab.key
                    ...
                    >
                        ...
                        {/* show label always on wider small screens, icon only on very narrow */}
...
export default function TournamentEditPage() {
                ...
                for (const r of roundRows as RoundRow[]) {
                    initial[r.number] = {
                        ...
                        startDate:            toDateStr(r.start_at),
                                             ...
                                             judgingDeadlineDate:  toDateStr(r.judging_deadline),
                                             judgingDeadlineTime:  toTimeStr(r.judging_deadline),
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        for (const r of roundSlices) {
            if (r.start && r.end && r.start >= r.end)
                addErr(`round_${r.n}_start`, `Раунд ${r.n}: початок повинен бути раніше за дедлайн здачі.`);
            if (ts && r.start && r.start < ts)
                addErr(`round_${r.n}_start`, fe[`round_${r.n}_start`] ?? `Раунд ${r.n}: початок раунду не може бути раніше за старт турніру.`);
            if (te && r.end && r.end > te)
                addErr(`round_${r.n}_end`, `Раунд ${r.n}: дедлайн здачі не може виходити за межі турніру.`);
            if (r.jd) {
                if (r.end && r.jd <= r.end)
                    addErr(`round_${r.n}_jd`, `Раунд ${r.n}: дедлайн оцінювання повинен бути пізніше за дедлайн здачі.`);
                if (te && r.jd > te)
                    addErr(`round_${r.n}_jd`, fe[`round_${r.n}_jd`] ?? `Раунд ${r.n}: дедлайн оцінювання не може виходити за межі турніру.`);
            }
        }
        for (let i = 0; i < roundSlices.length - 1; i++) {
            const cur = roundSlices[i];
            ...
            if (curEffectiveEnd && nxt.start && curEffectiveEnd > nxt.start) {
                const boundary = cur.jd ? "дедлайн оцінювання" : "дедлайн здачі";
                addErr(`round_${nxt.n}_start`, `Раунд ${nxt.n} починається до завершення раунду ${cur.n} (${boundary}).`);
            }
        }
        ...
        // Auto-switch to the relevant tab on validation error
        const hasRoundErrors = Object.keys(fe).some(k => k.startsWith('round_') || k === 'criteria');
                ...
                Array.from({ length: roundCount }, async (_, i) => {
                    ...
                    return {
                        ...
                        name:             rd?.name?.trim()                         || `Раунд ${n}`,
                           ...
                           judging_deadline: toIso(rd?.judgingDeadlineDate ?? "", rd?.judgingDeadlineTime ?? "") ?? null,
    ...
    return (
        ...
        <style>{`
            ...
            {bannerUrl ? (
                ...
                <button type="button" onClick={() => { setBannerError(""); setShowBannerEditor(true); }}
            ...
            <DateTimePair label={t.editTourney?.regStart ?? "Початок реєстрації"} dateVal={regFromDate} onDate={v => { setRegFromDate(v); clearFieldError("regFromDate"); }} timeVal={regFromTime} onTime={setRegFromTime} />
            <FieldError msg={fieldErrors.regFromDate} />
            <div className="border-t border-(--brd)" />
            <DateTimePair label={t.editTourney?.regEnd ?? "Кінець реєстрації"} dateVal={regToDate} onDate={v => { setRegToDate(v); clearFieldError("regToDate"); }} timeVal={regToTime} onTime={setRegToTime} />
            ...
            <div className="p-6 space-y-4 flex-1">
            <DateTimePair label={t.editTourney?.tourStart ?? "Початок турніру"} dateVal={startDate} onDate={v => { setStartDate(v); clearFieldError("startDate"); }} timeVal={startTime} onTime={setStartTime} required />
            ...
            {/* TIMELINE — горизонтальний скрол на вузьких екранах */}
            ...
            {roundCount === 1
                ...
                {/* ════════════════════════════════════════
                 *  RIGHT COLUMN: Rounds tab
                 *  - xl+: always visible (sticky)
                ...
                {/* ════════════════════════════════════════
                 *  JURY COLUMN
                 *  - xl+: hidden (jury lives inside left column via JuryInvitePanel below)
