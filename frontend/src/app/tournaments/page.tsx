...
interface Tournament {
    ...
    end_at?: string;
    team_count: number;
    banners?: string[]; // публічні URL банерів із Storage
}
...
const SECTIONS: SectionConfig[] = [
    { key: "ongoing",      label: "В процесі",  accent: "text-blue-500",   accentBg: "bg-blue-500/10",   accentBorder: "border-blue-500/20",   dot: "bg-blue-500",   defaultOpen: true  },
{ key: "registration", label: "Реєстрація", accent: "text-green-500",  accentBg: "bg-green-500/10",  accentBorder: "border-green-500/20",  dot: "bg-green-500",  defaultOpen: true  },
{ key: "upcoming",     label: "Скоро",      accent: "text-amber-500",  accentBg: "bg-amber-500/10",  accentBorder: "border-amber-500/20",  dot: "bg-amber-500",  defaultOpen: true  },
{ key: "finished",     label: "Завершено",  accent: "text-gray-400",   accentBg: "bg-gray-500/10",   accentBorder: "border-gray-500/20",   dot: "bg-gray-500",   defaultOpen: false },
];
...
function TournamentCard({
    ...
    const banners = tt.banners ?? [];

    // Слайдшоу якщо банерів більше одного
    ...
    return (
        <button
        onClick={onClick}
        className="w-full text-left rounded-2xl border border-(--brd) bg-(--card) p-4 sm:p-5 flex flex-col gap-3 hover:scale-[1.01] hover:border-blue-600/40 active:scale-[0.99] transition-all group shadow-sm overflow-hidden relative"
        >
        {/* ── Банери на фоні ── */}
        {banners.length > 0 && (
            ...
            {banners.map((url, i) => (
                ...
                style={{ opacity: i === bannerIdx ? 0.13 : 0 }}
                />
            ))}
            {/* затемнення по краях — текст залишається читабельним */}
            <div
            className="absolute inset-0"
            style={{
                background:
                "linear-gradient(to right, var(--card) 0%, transparent 40%, transparent 60%, var(--card) 100%), " +
                "linear-gradient(to bottom, var(--card) 0%, transparent 30%, transparent 70%, var(--card) 100%)",
            }}
            />
            </div>
        )}
        ...
        <p className={`relative z-10 text-base font-black text-(--t1) leading-tight group-hover:${cfg.accent} transition-colors truncate`}>
        {tt.name}
        </p>
        <div className="relative z-10 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-bold text-(--t2) uppercase tracking-wider">
...
function ActiveTournamentsList({
    tournaments, locale, searchQ, onOpen,
}: {
    tournaments: Tournament[]; locale: string; searchQ: string; onOpen: (id: string) => void;
}) {
    ...
    useEffect(() => {
        ...
        const fetchTournaments = async () => {
            ...
            try {
                ...
                // Завантажуємо банери з Storage (бакет "tournament-banners", папка = id турніру)
                const bannersMap: Record<string, string[]> = {};
                await Promise.all(
                    (tData ?? []).map(async (item: any) => {
                        try {
                            const { data: files } = await supabase.storage
                            .from("tournament-banners")
                            .list(item.id, { limit: 20 });
                            if (files && files.length > 0) {
                                bannersMap[item.id] = files
                                .filter(f => f.name && !f.name.startsWith("."))
                                .map(f => {
                                    const { data } = supabase.storage
                                    .from("tournament-banners")
                                    .getPublicUrl(`${item.id}/${f.name}`);
                                    return data.publicUrl;
                                });
                            }
                        } catch {
                            // банерів немає — нічого страшного
                        }
                    })
                );
...
// ─── Root export — wraps inner in Suspense ────────────────────────────────────
