...
import {
    ...
    CheckCheck, Crown, UserPlus, AlertCircle,
} from "lucide-react";
...
const typeIcon: Record<string, React.ReactNode> = {
    team_invitation:    <UserPlus size={16} className="text-blue-500" />,
    invitation_accepted:<Check    size={16} className="text-green-500" />,
    invitation_declined:<X        size={16} className="text-red-500" />,
};
        ...
        const respondInvitation = async (notif: Notification, accept: boolean) => {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    alert(err.detail ?? "Помилка відповіді на запрошення");
                    return;
                }
                ...
                setResponding(prev => ({ ...prev, [key]: null }));
            }
        };
        ...
        return (
            ...
            <style jsx global>{`
                ...
                {loading ? (
                    ...
                    <p className="text-(--t2) text-sm">Тут зʼявляться запрошення до команд та інші оповіщення</p>
                    </div>
                ) : (
...
function NotificationCard({
    ...
    return (
        <div
        className={`fuIn cdIn bg-(--card) rounded-2xl border border-(--brd) border-l-4 ${borderClass} p-5 sm:p-6 transition-all ${!notif.read ? "shadow-sm" : "opacity-70"}`}
        ...
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
            ...
            "bg-(--bg) border border-(--brd)"
        }`}>
        {typeIcon[notif.type] ?? <Bell size={16} className="text-(--t2)" />}
        </div>
        ...
        {isInvite && !responded && (
            ...
            onClick={onAccept}
            disabled={!!responding}
            className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-blue-600 text-white font-black text-[11px] uppercase tracking-widest hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-60 disabled:cursor-not-allowed shadow-md shadow-blue-600/25"
            >
            {responding === "accept"
                ? <><Loader size={12} className="animate-spin" /> Прийняття...</>
            ...
            onClick={onDecline}
            disabled={!!responding}
            className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-(--bg) border border-(--brd) text-(--t2) font-black text-[11px] uppercase tracking-widest hover:border-red-500/40 hover:text-red-500 active:scale-95 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            >
            {responding === "decline"
                ? <><Loader size={12} className="animate-spin" /> Відхилення...</>
                : <><X size={12} /> Відхилити</>
            }
            </button>
            </div>
