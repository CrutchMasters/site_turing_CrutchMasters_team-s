...
export default function SubmitPage() {
    ...
    const handleRemoveFile = (f: AttachedFile) => {
        if (f.isLocal) removeLocalFile(f.id);
        else removeRemoteFile(f);
    };
    ...
    const handleAction = async (isDraft: boolean) => {
        if (!user) { setSubmitError("Ви не авторизовані."); return; }
        ...
        <div className="flex h-screen overflow-hidden bg-(--bg) text-(--t1)">
            ...
            <main className="flex-1 flex flex-col overflow-y-auto relative z-10">
                ...
                <div className="p-6 max-w-5xl w-full mx-auto">
                    ...
                    <h1 className="text-2xl font-black text-(--t1) leading-tight mb-8">{round.name}</h1>
                                ...
                                <div className="flex flex-col gap-3">
                                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-(--brd) bg-(--bg) focus-within:border-blue-600/60 focus-within:ring-2 focus-within:ring-blue-600/10 transition-all">
                                        ...
                                        <input type="url" value={githubUrl} onChange={e => setGithubUrl(e.target.value)}
                                            disabled={isDraftLocked} placeholder="https://github.com/your/repo"
                                    ...
                                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-(--brd) bg-(--bg) focus-within:border-blue-600/60 focus-within:ring-2 focus-within:ring-blue-600/10 transition-all">
                                        ...
                                        <input type="url" value={youtubeUrl} onChange={e => setYoutubeUrl(e.target.value)}
                                            disabled={isDraftLocked} placeholder="https://youtube.com/watch?v=..."
                                            className="flex-1 bg-transparent text-sm text-(--t1) outline-none placeholder:text-(--t2) font-medium disabled:opacity-50" />
                                    </div>
                                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-(--brd) bg-(--bg) focus-within:border-blue-600/60 focus-within:ring-2 focus-within:ring-blue-600/10 transition-all">
                                        <Globe size={16} className="text-(--t2) flex-shrink-0" />
                                        <input type="url" value={liveUrl} onChange={e => setLiveUrl(e.target.value)}
                                            disabled={isDraftLocked} placeholder="https://your-demo.vercel.app"
                                            className="flex-1 bg-transparent text-sm text-(--t1) outline-none placeholder:text-(--t2) font-medium disabled:opacity-50" />
                                    </div>
                                </div>
                            </Card>
                                    ...
                                    <>
                                        <div className="relative h-2 rounded-full bg-(--brd) overflow-hidden mb-1">
                                            ...
                                            {progressPct > 0 && progressPct < 100 && (
                                                <div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-2 border-(--card) transition-all duration-1000"
                                                    style={{
                            ...
                            <Card>
                                ...
                                {!isDraftLocked && (
                                    <div
                                        ...
                                        <input ref={fileInputRef} type="file" multiple accept=".md,.markdown"
                                    ...
                                    <div className="flex flex-col gap-2 mt-3">
                                        {attachedFiles.map(f => (
                                            <div key={f.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-(--brd) bg-(--bg)">
                                                ...
                                                {!isDraftLocked && (
                                                    <button onClick={() => handleRemoveFile(f)}
