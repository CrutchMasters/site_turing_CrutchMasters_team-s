...
function TimeBlock({ value, label, urgent }: { value: number; label: string; urgent?: boolean }) {
    return (
        <div className="flex flex-col items-center gap-1.5 flex-1">
            <div className={`w-full py-3 sm:py-4 rounded-2xl border flex items-center justify-center ${urgent ? "bg-red-500/10 border-red-500/25" : "bg-(--bg) border-(--brd)"}`}>
                <span
                    className={`text-2xl sm:text-3xl font-black tabular-nums ${urgent ? "text-red-500" : "text-(--t1)"}`}
...
export default function RoundPage() {
    ...
    // ── Mobile sidebar state lives here now ──
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (roundData.tournament_id) {
                ...
                .maybeSingle();
                if (tournErr) console.warn("tournaments fetch error:", tournErr.message);
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-(--bg) text-(--t1)">
            ...
            {/* Sidebar — hidden on mobile, shown as drawer when mobileOpen */}
            <Sidebar
                mobileOpen={isMobileSidebarOpen}
                onMobileClose={() => setIsMobileSidebarOpen(false)}
            />
            ...
            <main className="flex-1 flex flex-col overflow-y-auto relative z-10 min-w-0">
                <MobileHeader
                ...
                <div className="p-4 sm:p-6 max-w-5xl w-full mx-auto">

                    {/* back */}
                    <a
                        href={`/tournaments/${round.tournament_id}`}
                        onClick={(e) => { e.preventDefault(); router.push(`/tournaments/${round.tournament_id}`); }}
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-3">
                        ...
                        {tournament && (
                            <span className="text-[11px] font-bold text-(--t2) truncate max-w-[160px] sm:max-w-none">{tournament.name}</span>
                        )}
                    </div>

                    <h1 className="text-xl sm:text-2xl font-black text-(--t1) leading-tight mb-6 sm:mb-8 break-words">
                        {round.name}
                    </h1>

                    {/* main grid — single column on mobile, two columns on lg */}
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <Card>
                                ...
                                <div className="flex items-end gap-1.5 sm:gap-2 mb-5">
                                    <TimeBlock value={countdown.days}    label="днів"  urgent={isUrgent} />
                                    <span className="text-xl sm:text-2xl font-black text-(--t2) mb-6 flex-shrink-0">:</span>
                                ...
                                {endTs > 0 && (
                                    <>
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="text-[10px] sm:text-xs text-(--t2) font-bold flex items-center gap-1 sm:gap-1.5 min-w-0">
                                                <Calendar size={12} className="flex-shrink-0" />
                                                <span className="truncate">{round.start_at ? fmtDate(round.start_at) : "Старт не вказано"}</span>
                                            </span>
                                            <span className="text-[10px] sm:text-xs text-(--t2) font-bold flex items-center gap-1 sm:gap-1.5 min-w-0">
                                                <span className="truncate">{fmtDate(round.end_at)}</span>
                                                <Calendar size={12} className="flex-shrink-0" />
                                            </span>
                                        </div>
                                    </>
