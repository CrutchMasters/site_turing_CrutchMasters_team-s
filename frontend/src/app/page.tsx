...
export default function Home() {
  ...
  const { dark, toggle } = useTheme(); // Твоя нова функція
  ...
  return (
    // Тепер використовуємо прозорий фон, бо основний колір задано в layout
    ...
    <style jsx global>{`
      ...
      <div className={`absolute bottom-16 right-0 w-64 bg-(--card)/80 backdrop-blur-2xl rounded-3xl shadow-2xl border border-(--brd) p-5 transition-all duration-300 origin-bottom-right ${isSettingsOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4 pointer-events-none'}`}>
      <h3 className="text-xs font-black uppercase tracking-widest text-(--t2) mb-4 px-1">{t.settings.title}</h3>
      <div className="space-y-4">

      {/* Кнопка теми інтегрована в існуючий список */}
      ...
      <span className="text-[10px] font-black text-(--t2) uppercase tracking-tighter block mb-1">
      ...
      className={`p-4 rounded-2xl bg-(--card) shadow-xl border border-(--brd) transition-all hover:scale-110 ${isSettingsOpen ? 'rotate-90 text-blue-600' : 'text-(--t2)'}`}
      >
      ...
      <header ref={headerRef} className="plasma-menu opacity-0 -translate-y-full w-full max-w-7xl bg-(--card)/70 backdrop-blur-xl py-3 px-8 flex justify-between items-center rounded-2xl shadow-lg border border-(--brd)">
      <div className="flex items-center gap-3">
      <Image src="/logo_homepage.svg" alt="Logo" width={28} height={28} priority className={dark ? "invert" : ""} />
      <span className="text-lg font-bold tracking-tight text-(--t1) uppercase">CodeFuture</span>
      </div>
      <div className="flex items-center gap-4">
      <Link href="/login" className="text-sm font-semibold text-(--t2) hover:text-blue-600 transition px-3 py-1">{t.nav.signIn}</Link>
      <Link href="/register">
      ...
      <div ref={(el) => { revealRefs.current[0] = el; }} className="reveal-drop opacity-0 -translate-y-10 bg-(--card)/95 backdrop-blur-sm p-10 rounded-[2.5rem] shadow-xl border border-(--brd) flex flex-col">
      ...
      <h2 className="text-2xl font-black mb-4 text-(--t1) uppercase tracking-tight">{t.hero.ecosystem}</h2>
      <p className="text-(--t2) leading-relaxed text-sm mb-6">
      ...
      <button onClick={scrollToContent} className="flex flex-col items-center gap-2 text-(--t2) hover:text-blue-600 transition-colors group">
      ...
      <div ref={(el) => { revealRefs.current[2] = el; }} className="reveal-drop opacity-0 -translate-y-10 bg-(--card)/95 backdrop-blur-sm p-10 rounded-[2.5rem] shadow-xl border border-(--brd) flex flex-col">
      <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white mb-6">
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
      </div>
      <h2 className="text-2xl font-black mb-4 text-(--t1) uppercase tracking-tight">{t.hero.functionality}</h2>
      <p className="text-(--t2) leading-relaxed text-sm mb-6">{t.hero.funcDesc}</p>
      </div>
      </div>
      </main>
      ...
      {sections.map((item: any, index: number) => (
        ...
        className="reveal-drop opacity-0 -translate-y-10 bg-(--card) p-10 rounded-[2.5rem] shadow-sm border border-(--brd)"
        >
        <h2 className="text-xl font-black mb-3 text-(--t1) uppercase tracking-tight">{item.title}</h2>
        <p className="text-(--t2) text-base leading-relaxed">{item.text}</p>
        </div>
      ))}
      </section>

      <footer className="py-12 border-t border-(--brd) flex flex-col items-center gap-3 opacity-30 mt-auto">
      <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-(--t2)">Powered by Crutch Masters</span>
      </footer>
      </div>
  );
}
