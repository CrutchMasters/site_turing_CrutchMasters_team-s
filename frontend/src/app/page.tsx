...
import { useAuth } from "@/context/AuthContext"; // Використовуємо єдиний правильний шлях
import {
...
export default function HomePage() {
  ...
  const { user, logout, isLoading } = useAuth(); // Додано isLoading для запобігання помилок рендерингу
  ...
  return (
    ...
    <style jsx global>{`
      ...
      <div className={`
        ...
        <img src="/logo_homepage.png" alt="Logo" width={24} height={24} className="flex-shrink-0" />
        <span className="text-sm sm:text-lg font-bold tracking-tight text-(--t1) uppercase truncate">CodeFuture</span>
        </div>
        ...
        {user ? (
          ...
          <span className="text-sm font-bold text-(--t1) hidden sm:block max-w-[120px] truncate">{user.username}</span>
          <ChevronRight className={`w-3 h-3 text-(--t2) transition-transform rotate-90 ${isUserMenuOpen ? "rotate-[270deg]" : ""}`} />
          </button>
          ...
          <>
          <Link href="/login" className="text-xs sm:text-sm font-semibold text-(--t2) hover:text-blue-600 transition px-2 sm:px-3 py-1">{t.nav.signIn}</Link>
          <Link href="/register">
          <button className="px-3 py-1.5 sm:px-5 sm:py-2 text-xs sm:text-sm font-bold text-white bg-blue-600 rounded-lg sm:rounded-xl hover:bg-blue-700 transition-all active:scale-95">{t.nav.signUp}</button>
          </Link>
          </>
        )}
        </div>
        ...
        <img src="/logo_background1.png" alt="Watermark" className={`w-[min(800px,90vw)] h-[min(800px,90vw)] object-contain ${dark ? "invert" : ""}`} />
        </div>
        ...
        {/* Функціонал */}
        <div ref={(el) => { revealRefs.current[2] = el; }} className="reveal-drop opacity-0 -translate-y-10 bg-(--card)/95 backdrop-blur-sm p-6 sm:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-xl border border-(--brd) flex flex-col">
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white mb-4 sm:mb-6"><Home size={24} /></div>
        <h2 className="text-lg sm:text-2xl font-black mb-2 sm:mb-4 text-(--t1) uppercase tracking-tight">{t.hero.functionality}</h2>
        ...
        {sections.map((item: any, index: number) => (
          <div key={index} ref={(el) => { revealRefs.current[index + 3] = el; }} className="reveal-drop opacity-0 -translate-y-10 bg-(--card) p-6 sm:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-(--brd)">
          <h2 className="text-base sm:text-xl font-black mb-2 sm:mb-3 text-(--t1) uppercase tracking-tight">{item.title}</h2>
