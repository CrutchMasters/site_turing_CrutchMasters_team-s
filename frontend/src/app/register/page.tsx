...
export default function RegisterPage() {
  ...
  // ОБНОВЛЕННАЯ ФУНКЦИЯ: теперь она реально отправляет данные на бэкенд
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (canSubmit) {
      try {
        ...
        alert("Не удалось связаться с сервером. Проверь, запущен ли FastAPI на порту 8000.");
      }
    }
  };
  ...
  return (
    ...
    className={`w-full px-4 py-3 rounded-xl border bg-white focus:ring-2 outline-none transition-all placeholder:italic text-sm ${
      !isPasswordMatch && formData.confirmPassword ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-blue-500"
    }`}
    required
    />
    </div>
    ...
    className={`w-full py-4 rounded-2xl text-2xl font-black shadow-lg transition-all active:scale-[0.97] uppercase tracking-tight ${
      canSubmit ? "bg-blue-600 text-white hover:bg-blue-700 cursor-pointer shadow-blue-200" : "bg-gray-300 text-gray-500 cursor-not-allowed opacity-70"
    }`}
    >
    Registered
    </button>
