...
export default function JuryEvaluationPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // 2. Submissions assigned to this jury member
            const { data: submissionsData } = await supabase
                .from("submissions")
                .select("id, team_id, round_id, submitted_at, github_url, video_url")
                .eq("round_id", roundId);
            ...
            // 3. For jury: filter only assigned submissions
            let assignedSubmissions = submissionsData;
            if (isJury) {
                const { data: assignments } = await supabase
                    ...
                    .eq("jury_id", user.id)
                    .eq("round_id", roundId);
                const assignedIds = new Set((assignments ?? []).map((a: any) => a.submission_id));
                assignedSubmissions = submissionsData.filter(s => assignedIds.has(s.id));
            }

            // 4. Load team names
            const teamIds = [...new Set(assignedSubmissions.map(s => s.team_id))];
            const { data: teamsData } = await supabase
                .from("teams")
                .select("id, name, city_school_org")
                .in("id", teamIds);
            const teamMap: Record<string, { name: string; org?: string }> = {};
            (teamsData ?? []).forEach((t: any) => { teamMap[t.id] = { name: t.name, org: t.city_school_org }; });

            // 5. Load existing evaluations
            const submissionIds = assignedSubmissions.map(s => s.id);
            ...
            // 6. Build work list
            const workList: SubmissionWork[] = assignedSubmissions.map(s => {
                ...
                return {
                    id: s.id,
                    team_id: s.team_id,
                    team_name: teamMap[s.team_id]?.name ?? "Команда",
                    team_org: teamMap[s.team_id]?.org,
                    round_id: s.round_id,
